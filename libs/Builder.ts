...
export class Builder {
    ...
    static fromConfig(treeConfig: Builder.ITreeConfig[]): PIXI.Container[] {
        return treeConfig.map((config) => {
            const methodName: Builder.Methods = Builder.methodsMap[config.type];
    ...
    private static _useModifiers<T extends PIXI.DisplayObject>(target: T, modifiers: Builder.IModifiers = {}): void {
        const filtered = Builder.modifiersList.filter((prop) => {
    ...
    private static _modify<T extends PIXI.DisplayObject>(target: T, property: string, modifier: Builder.Modifier): void {
